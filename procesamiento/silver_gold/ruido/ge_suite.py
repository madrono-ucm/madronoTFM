"""Puerta de calidad Great Expectations del dataset `ruido` (Silver).
...
A diferencia de `calidad_aire`/`meteorologia` (un rango de plausibilidad por
etiqueta, que necesita una columna auxiliar porque GX no tiene una
expectation nativa de "el rango depende de otra columna"), aquí
`transform.PLAUSIBLE_DB_RANGE` es un único rango global para los seis campos
de nivel sonoro -- se traduce directamente a
`expect_column_values_to_be_between` sobre cada columna, sin ninguna
columna auxiliar. Los percentiles (`l1_db`..`l99_db`) pueden ser nulos (ver
`transform.py`): GX ya excluye los valores nulos de
`expect_column_values_to_be_between` por defecto (solo evalúa el rango sobre
los valores no nulos), así que no hace falta ningún `mostly` distinto de
1.0 ni ninguna comprobación adicional para permitirlo.
"""
...
from procesamiento.silver_gold.ruido.transform import PLAUSIBLE_DB_RANGE

EXPECTATION_SUITE_NAME = "ruido_silver_suite"
...
def build_validator(context: "gx.data_context.AbstractDataContext", silver_df: "DataFrame"):
    ...
    del patrón.
    """
    datasource = context.sources.add_or_update_spark(name="glue_ruido_silver")
    data_asset = datasource.add_dataframe_asset(name="ruido_silver")
    ...
    # Reproduce transform.validate_record: "period_missing".
    validator.expect_column_values_to_not_be_null("period")
    # Reproduce transform.validate_record: "measured_date_missing_or_unparseable".
    validator.expect_column_values_to_not_be_null("measured_date")
    ...
    # Reproduce transform.validate_record: "laeq_missing" y
    # "laeq_db_out_of_plausible_range".
    validator.expect_column_values_to_not_be_null("laeq_db")
    validator.expect_column_values_to_be_between("laeq_db", min_value=min_db, max_value=max_db, mostly=1.0)

    # Reproduce transform.validate_record: "<percentil>_out_of_plausible_range"
    # -- nulos permitidos (ver docstring del módulo), solo se acota el rango.
    for field in ("l1_db", "l10_db", "l50_db", "l90_db", "l99_db"):
        validator.expect_column_values_to_be_between(field, min_value=min_db, max_value=max_db, mostly=1.0)
