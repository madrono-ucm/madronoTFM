...
| `calidad_aire_cams(contaminante, fecha=None)` | previsión del modelo atmosférico **Copernicus CAMS** (nivel ciudad) — 2.ª opinión independiente; `calidad_aire_prevista` la trae como `referencia_cams`/`delta_vs_cams` | `FIL_80` |
| `meteo_cercana(lugar, radio_m=1500, fecha=None)` | meteorología observada (temp/viento/precip/humedad) en la estación `:EstacionMedida{meteo}` más cercana del grafo + `gold.meteorologia_por_estacion_magnitud_hora`; sin `fecha`, fiabilidad `BAJA` por frescura (pipeline congelado), `MEDIA` con `fecha` explícita (`FIL_88`, mismo contrato que `avisos_meteo`) | `FIL_82` |
| `avisos_meteo(zona=None, fecha=None)` | avisos meteorológicos AEMET activos (nivel más alto + fenómenos + vigencia) de `gold.aemet_avisos_por_zona_fecha_nivel` | `FIL_82` |
