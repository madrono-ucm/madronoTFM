...
**Estado (tarea 091): solo `opciones_movilidad` sigue pendiente.** Este
...
`eventos_cercanos` (tarea 091) resuelve el lugar contra el grafo (sin seguir
...
por categoría/distrito/fecha sin lat/lon por evento, ver `doc/091-...md`) --
...
asistente/
  ...
  neo4j_client.py                 # run_neo4j_query() + query builders de trafico_cercano (081)/afluencia_estimada (089)/eventos_cercanos (091)
  routers/
    ...
    eventos_cercanos.py                     # GET /eventos-cercanos -- ídem, tarea 091 (lista, no un único modelo)
  models/
  ...
  tests/
    ...
    test_eventos_cercanos_router.py              # GET /eventos-cercanos (mockeando Athena y Neo4j), tarea 091
...
que `eventos_cercanos` (tarea 091) resuelve el lugar y filtra por distancia
...
comprobado en la tarea 091 que `NEO4J_URI`/`NEO4J_USERNAME`/
...
**`eventos_cercanos` (tarea 091)**: arrancado el servicio real con
...
0. ~~(Tarea 081) Verificar `trafico_cercano` contra la instancia real de
   ...
   091, ver "Verificación real" arriba) -- sigue pendiente repetir la
...
1. Implementar la última `tool` (`opciones_movilidad` -- `afluencia_prevista`
   ...
   ya es real, tarea 091), mismo patrón que las demás (tool en `tools.py` +
