...
def consultar_ruta_saludable(
    ...
    cbi = r.cambio_exposicion_pct
    signo = lambda k: ("−" if cbi.get(k, 0) >= 0 else "+") + f"{abs(cbi.get(k, 0)):.0f}%"
    explicacion = (
        ...
        f"sobre la ruta rápida) para −{r.reduccion_exposicion_pct:.0f}% de exposición ponderada. "
        f"Por señal: tráfico {signo('traf')}, NO₂ {signo('no2')}, O₃ {signo('o3')}, ruido {signo('noise')} "
        f"(la ruta canjea unas señales por otras). Mejor hora de salida: {r.mejor_hora_salida:02d}:00. "
