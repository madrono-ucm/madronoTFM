...
apartado 6.1, categoría «Medioambiente»: meteorología). Sigue el mismo patrón que la
tarea 002 (`ingesta/capturas/trafico_madrid.py`).
...
Investigar y usar una fuente de datos abiertos meteorológicos de Madrid — el
Ayuntamiento de Madrid publica datos en tiempo real de su red de estaciones
meteorológicas en datos.madrid.es (sin autenticación); AEMET OpenData es una
alternativa/complemento pero requiere una API key gratuita con registro. Usa tu
criterio y documenta la fuente elegida.

## Alcance concreto

1. Crea `ingesta/capturas/meteorologia_madrid.py` siguiendo el mismo patrón que
   `trafico_madrid.py`: descarga -> normaliza a un esquema mínimo y consistente
   (timestamp, id de estación, ubicación, temperatura, humedad, viento, precipitación
   — los campos que ofrezca la fuente elegida).
2. Si usas AEMET, lee la API key de una variable de entorno, sin hardcodearla, y
   documenta cómo obtenerla.
...
- No uses APIs de pago. Una API key gratuita con registro (p.ej. AEMET) es aceptable.
