...
| `FIL_12` | `FIL_09` verificó solo frescura por fecha — 6 datasets con ~20 h perdidas el 29/8 durante el incidente, nunca rellenadas | ✅ **HECHO 30/8** — decisión: **limitación documentada, no backfill** (ningún `bronze_to_silver` acepta hora objetivo → ~120 runs + cambios en ~12 jobs para un hueco de 1 día; el panel de ML ya trata huecos horarios como NaN). Bronze está intacto. `doc/FIL-09` §"Completitud por hora del 29/8"; nota para memoria §7.4 vía `VIKT_*` |
