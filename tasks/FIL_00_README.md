...
| `FIL_32`–`FIL_36` | **Mapa animado del grafo de Madrid** (elemento "wow" del TFM) — export del grafo canónico, `prevision_animada.parquet`, HTML animado (pydeck), capas ricas + hosting en Pages, el grafo como eje de la memoria | ✅ **HECHO 31/8** — spine M1–M6 en un día (PRs #209–#216). `viz/`, live en `https://madrono-ucm.github.io/madronoTFM/`. Ver `viz/PROGRESO_MAPA.md`. |
| `FIL_37` | `ruta_saludable` — recomendador ambiental sobre el grafo (**12.ª tool MCP**) + evaluación Pareto | ✅ **HECHO 31/8** (PR #211 + #213) — `viz/rutas.py` + `asistente/ruta_saludable.py` (Dijkstra Python puro) + `grafo_ruta.json`. `fiabilidad` BAJA (§7.4). |
| `FIL_38` | Backtest offline más largo con MTD (~29 meses, 300 sensores) | ✅ **HECHO 31/8** (PR #215) — STGNN bate persistencia: skill **+0,37 / +0,70 / +0,85** a h1/h3/h6. Results-only. `doc/FIL-38`. |
| `FIL_42` | Publicar el mapa en GitHub Pages (rama `gh-pages` huérfana) | ✅ **HECHO 31/8** (PR #212). |
| `FIL_43` | `ruta_saludable`: la reducción reportada ≠ la que Dijkstra minimiza (salía negativa) | ✅ **HECHO 31/8** (PR #214) — se reporta `E_ponderada` (lo optimizado), nunca negativa, + `cambio_por_senal_pct`. |
| `FIL_44` | `gh-pages` con `rutas.json` viejo tras el fix de `FIL_43` | ✅ **HECHO 31/8** (PR #217) — falsa alarma, carrera de despliegue, verificado en vivo. |
| `FIL_45` | Mapa — **capa social y de accesibilidad**: perfiles de sensibilidad, color por banda OMS/UE, dosis, "mejor hora", confianza IDW, UI español + AA | ⬜ **Pendiente** — M4b, objetivo 09-11. Amplía `FIL_33`/`FIL_34`. Cero AWS. |
| `FIL_46` | Acceso en lenguaje natural a la capa social + alertas anticipadas por distrito | 🔲 **Encuadre** — no implementado; sustrato hecho, falta canal de notificación + política de umbral. Se cita en `FIL_36`. |
| `FIL_47` | Mapa — **legibilidad**: cámara 2D/3D, etiquetas de distrito, hitos, ejes principales, parques | ⬜ **Pendiente** — M4c, objetivo 09-12. Depende de `FIL_34`/`FIL_35`. Cero AWS. |
