...
| `FIL_11` | `ruido` (11 días) y `aemet_avisos` (8 días) con Gold estancado pese a jobs `SUCCEEDED` a diario — mismo patrón de escritura vacía silenciosa que `FIL_01`/tareas 072/090 | ⬜ **Hallazgo de QA (`VIC_09`), sin investigar la causa raíz exacta ni tocar código** — Bronze y Silver frescos en ambos casos (con datos reales, no "sin novedades"), pero Gold no avanza; hipótesis razonada en el ticket, pendiente de confirmar leyendo `glue_bronze_to_silver.py` de cada dataset y proponer el fix como ticket aparte |
| `FIL_12` | `FIL_09` se declaró resuelto verificando solo frescura por fecha — 6 datasets (`trafico`/`bicimad`/`transporte_publico_emt`/`meteorologia`/`calidad_aire`/`aparcamientos`) tienen 17-21 horas perdidas el 29/8, nunca rellenadas | ⬜ **Hallazgo de QA (`VIC_11`)** — confirmado con `aws glue get-job-runs` que `trafico_bronze_to_silver` falló 19 ejecuciones horarias consecutivas durante el incidente; el fix restauró el pipeline hacia delante pero nunca hizo backfill de las horas perdidas. Decisión pendiente: backfill real o limitación documentada |
