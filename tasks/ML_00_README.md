...
| `ML_10` | **Tier 4**: reentrenamiento nocturno programado + backtest incremental (los datos crecen hasta la entrega) | ✅ **HECHO** — `evaluation/backtest.py` (rolling origin, curva skill vs fecha) + `training/retrain_nightly.py` (reentrena+evalúa+MLflow+promueve si no regresa). Mecanismo: cron en la EC2 (coste 0, sin Terraform). Ejecución real 29/8. `doc/ML-10` |
