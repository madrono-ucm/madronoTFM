...
- ~~`eventos_cercanos`~~ **Hecho (tarea 093)** — resuelve el lugar contra el
  ...
  deduplicación por `event_id`), ver `doc/093`
- ~~Persistir `NEO4J_URI`/`NEO4J_USERNAME`/`NEO4J_PASSWORD` como parámetros
  SSM `SecureString`~~ **Ya existen** (confirmado en la tarea 093,
  `/madrono-tfm/dev/secrets/neo4j-*`) — el gap de la tarea 043 se cerró en
  algún momento entre la 081 y la 093, sin documentarlo aquí. Sigue
