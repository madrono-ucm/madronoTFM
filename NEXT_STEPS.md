...
|---|---|---|---|
| 1 | **`modelado/` — fundación (Tier 0)**: feature store, arnés de CV temporal, líneas base, MLflow, Evidently, export ONNX | Sistema | ✅ **hecha** — `ML_02`–`ML_10` ✅ (ver filas 4/5/7); `ML_01` (feature store) ✅ **cerrado 29/8**: meteo observada (join espacial) + previsión AEMET (Silver, sin fuga) + festivos (fichero real) — panel real a 30 features, `doc/ML-01`. Único resto: `afluencia` espera horas en la Gold de FIL_06 |
