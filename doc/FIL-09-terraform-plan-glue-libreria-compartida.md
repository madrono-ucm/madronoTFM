# FIL-09 — Plan de Terraform para reparar 37/48 jobs de Glue rotos por una librería compartida inexistente

**Tarea deliberadamente de solo lectura hasta este punto** (el plan se generó y se
revisa aquí; `allow_infra_apply` en `FIL_09` queda en `true` porque esta ficha SÍ
incluye la recomendación de aplicar, pero **la ejecución del `apply` requiere
aprobación humana explícita antes de lanzarse**, mismo criterio que las tareas
098/100). Sigue el patrón de dos pasos de `tasks/README.md`: este documento es el
"plan" para revisión; `FIL_09` en sí ya incluye también los pasos de "apply" y
verificación, a ejecutar solo tras luz verde humana.

## Resumen ejecutivo

- **37 de 48 jobs de Glue (77 %) fallan en `LAUNCH ERROR`** desde al menos el
  2026-08-28 15:13 (>28 horas continuas a fecha de este informe, 2026-08-29
  ~19:30 UTC) porque su argumento `--extra-py-files` apunta a un fichero
  `procesamiento-<hash>.zip` en S3 que **no existe**.
- Verificado en vivo (`aws glue get-jobs`, `aws s3 ls`): 27 jobs apuntan a un hash
  inexistente, 10 a otro hash distinto también inexistente, y solo 1 job apunta al
  único objeto real que hay en el bucket (subido el 28/8 15:19:29). El `main`
  actual del repositorio ya calcula un **cuarto** hash, distinto de los tres
  anteriores, todavía sin desplegar.
- **Terraform state coincide con el único objeto real** — confirma que el origen
  no es un problema de Terraform en sí, sino de **aplicaciones parciales previas**
  (`-target=...` sobre subconjuntos de recursos, mismo patrón que causó el
  incidente de finales de línea de la tarea 100) que dejaron distintos jobs
  "anclados" a generaciones ya borradas del fichero compartido.
- **Impacto real**: Bronze→Silver roto para `trafico`, `bicimad`,
  `transporte_publico_emt`, `meteorologia`, `calidad_aire` y `aparcamientos` — 6 de
  los 16 "productores en producción continua" que describe la memoria
  (`VIC_02`/`VIKT_03`). Confirmado de forma independiente: la verificación en vivo
  de la tool `calidad_aire_prevista` (misma sesión de QA) encontró que Gold de
  calidad del aire no tiene ninguna fila posterior a 2026-08-28 15:00 — coincide
  exactamente con el inicio de la rotura.
- **El plan de reconciliación (abajo, íntegro) es seguro**: verificado línea a
  línea con `grep` que **no hay ninguna destrucción "suelta"** — cada uno de los
  49 `destroy` es la mitad-baja de un par `must be replaced` (se sustituye el
  objeto S3 antiguo por uno nuevo con el contenido real de `main`, nunca se borra
  algo sin reemplazo). Resumen: **49 to add, 66 to change, 49 to destroy**
  (Kafka, tarea 042, excluido a propósito del `-target`, igual que la tarea 098 —
  nunca se ha aplicado y este plan no lo toca).
- Desglose por tipo de recurso: 45 `aws_s3_object.glue_script_*`/
  `procesamiento_source`/`layer_build_source` reemplazados (nuevo hash de
  contenido); 14 `aws_lambda_function.producer[*]` actualizadas en el sitio (nuevo
  `source_code_hash`, mismo paquete compartido); 44 `aws_glue_job.*` actualizados
  en el sitio (nuevo `--extra-py-files`/`script_location` apuntando a las claves
  S3 correctas); 1 `aws_codebuild_project` y 1 `aws_iam_policy` actualizados por
  dependencia.

## Recomendación

1. Que un humano revise este documento (el resumen de arriba y, si quiere más
   detalle, el plan completo más abajo).
2. Aprobar explícitamente la ejecución del `apply` — es el paso más urgente del
   proyecto ahora mismo: cada hora que pasa es más histórico perdido en 6
   datasets "de producción continua" y una desviación mayor entre lo que dice la
   memoria y lo que hay realmente en Gold.
3. Tras el `apply`, verificar en vivo (no solo el código de salida):
   - `aws glue get-jobs` — todos los `--extra-py-files` apuntan al mismo hash, y
     ese objeto existe en S3.
   - Lanzar o esperar al siguiente disparo programado de al menos uno de los 6
     jobs `bronze_to_silver` que fallaban, confirmar `SUCCEEDED`.
   - Athena: `calidad_aire`/`trafico`/etc. con filas de fecha/hora posteriores a
     2026-08-28 15:00.
4. Documentar el resultado en `doc/FIL-09-...md` (añadir una sección al final de
   este mismo fichero, no crear uno nuevo) y actualizar `PLAN.md`
   (bloqueador 0) y `NEXT_STEPS.md`.

## Advertencia (heredada de la tarea 098)

`terraform plan -destroy -target=...` sobre un solo dataset puede arrastrar, por
políticas IAM compartidas, la planificación de destruir recursos de otros
productores — no uses `-destroy -target` sin entender antes el grafo de
dependencias. Este plan ya está generado sin ese patrón (usa `-target` solo para
excluir Kafka de un plan normal, no para destruir nada).

## Cómo se generó este plan (reproducible)

```bash
cd infra/terraform
# limpiar __pycache__ local antes de nada (tarea 092: fileset sin excluirlo
# rompe terraform plan con __pycache__ presente)
...
# excluir Kafka (nunca aplicado, tarea 042/098) del plan sin usar -exclude
# (no soportado en esta versión de Terraform):
...
data.archive_file.ingesta_source: Read complete after 3s [id=d585b4356f099df223cd692b481b25046f8bcdad]
...
data.aws_iam_policy_document.athena_results_bucket_policy: Read complete after 1s [id=3728792540]
...
aws_s3_object.layer_build_source: Refreshing state... [id=source/ingesta-requirements-c25e831d1b28851d8cdce0447b7a3aae.zip]
...
data.aws_iam_policy_document.glue_transporte_publico_emt_data_access: Read complete after 1s [id=1469263382]
...
data.archive_file.procesamiento_source: Still reading... [00m11s elapsed]
...
data.archive_file.procesamiento_source: Read complete after 13s [id=ae8c8f673275334b6e3c720d48940d7399264c78]
...
aws_s3_object.procesamiento_source: Refreshing state... [id=glue-libs/procesamiento-a7ba99ac9375c8f81e041af4185355c9.zip]
...
Terraform will perform the following actions:
 ...
 <= data "aws_iam_policy_document" "scheduler_invoke_lambda" {
  ...
  ~ resource "aws_codebuild_project" "lambda_dependencies_layer" {
      ...
      ~ source {
          ~ buildspec           = <<-EOT
            ...
            EOT
          ~ location            = "madrono-tfm-dev-build-artifacts-222234418587/source/ingesta-requirements-c25e831d1b28851d8cdce0447b7a3aae.zip" -> "madrono-tfm-dev-build-artifacts-222234418587/source/ingesta-requirements-65d5d59fef3021bc0831a29454145c7b.zip"
  ...
  ~ resource "aws_glue_job" "aemet_prevision_avisos_bronze_to_silver" {
      ~ default_arguments         = {
          ~ "--extra-py-files"                   = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-libs/procesamiento-72e35fd92f5084c4a801871ea7af5b62.zip" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-libs/procesamiento-38ac40cf3a7a2950bd8beefe827db5b5.zip"
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aemet_prevision_avisos_bronze_to_silver-d7b98621b0e2b5a6d5d4c70119146f34.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aemet_prevision_avisos_bronze_to_silver-5874411ad6f28f158f685ce90841add5.py"
  ...
  ~ resource "aws_glue_job" "aemet_prevision_avisos_silver_to_gold" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aemet_prevision_avisos_silver_to_gold-e203db858089238019b642eb9b9f6a23.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aemet_prevision_avisos_silver_to_gold-735e6a4f6938be2c1d4d15ce74735529.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "afluencia_lugares_bronze_to_silver" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/afluencia_lugares_bronze_to_silver-2299183b4c60edf1e2539fc00a59ccc5.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/afluencia_lugares_bronze_to_silver-e019ce6127891d24af49cb253082177b.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "afluencia_lugares_silver_to_gold" {
      ~ default_arguments         = {
          ~ "--extra-py-files"                   = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-libs/procesamiento-a7ba99ac9375c8f81e041af4185355c9.zip" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-libs/procesamiento-38ac40cf3a7a2950bd8beefe827db5b5.zip"
  ...
  ~ resource "aws_glue_job" "aforos_peatones_bicicletas_bronze_to_silver" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aforos_peatones_bicicletas_bronze_to_silver-d8325aae3ee77630cfc0f6612c30323e.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aforos_peatones_bicicletas_bronze_to_silver-d037fa3c1d50aa6cdf1a4355e4af910b.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "aforos_peatones_bicicletas_gold_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aforos_peatones_bicicletas_backfill_dedup_gold-7c312ea88152d382dd1e02b82a549a7c.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aforos_peatones_bicicletas_backfill_dedup_gold-dedd44063c766e8475986650df404498.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "aforos_peatones_bicicletas_silver_backfill_dedup" {
      ~ default_arguments         = {
          ~ "--extra-py-files"                   = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-libs/procesamiento-1ba560b736e3b3efe1bd2ebb6b171484.zip" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-libs/procesamiento-38ac40cf3a7a2950bd8beefe827db5b5.zip"
            # (9 unchanged elements hidden)
        }
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aforos_peatones_bicicletas_backfill_dedup-bfb76e782afee2c5956f548a34da0b18.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aforos_peatones_bicicletas_backfill_dedup-8dc97077e9a7b0d793edb42e68c8a090.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "aforos_peatones_bicicletas_silver_to_gold" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aforos_peatones_bicicletas_silver_to_gold-a7df88f8aa89cea895c3e594ff738600.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aforos_peatones_bicicletas_silver_to_gold-98ae6a2fa1ca9fc05b6451aaffbd690b.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "agenda_eventos_bronze_to_silver" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/agenda_eventos_bronze_to_silver-75c29ecd15eb33bf665840234bcf5cc8.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/agenda_eventos_bronze_to_silver-e24cfdad3be04dfe065231b9643719c0.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "agenda_eventos_gold_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/agenda_eventos_backfill_dedup_gold-c417e4a5711e3fcb2d416cc0f05f3290.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/agenda_eventos_backfill_dedup_gold-b4c8693ee116e2f50aee1b96fd7018c6.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "agenda_eventos_silver_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/agenda_eventos_backfill_dedup-ebb7fb05697677064a5b18ee492aca9e.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/agenda_eventos_backfill_dedup-406949108f74a212bfa3dd0a5f67acca.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "agenda_eventos_silver_to_gold" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/agenda_eventos_silver_to_gold-4d854fda4df74abc055b9eba9af92b0f.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/agenda_eventos_silver_to_gold-7ed6c1455ead3aef19f9e40b96c23a51.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "aparcamientos_bronze_to_silver" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aparcamientos_bronze_to_silver-4c9fe8e66729a98c520c97a0aa10f630.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aparcamientos_bronze_to_silver-90e5ef17131a899ea2f70fcff0bb1962.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "aparcamientos_gold_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aparcamientos_backfill_dedup_gold-7f4c18ec21a262d4a6e788348e492c3f.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aparcamientos_backfill_dedup_gold-b03433c6e4e72c7e33e557790f1809b2.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "aparcamientos_silver_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aparcamientos_backfill_dedup-0040b8ac53f09f609005c2ad2aac464f.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aparcamientos_backfill_dedup-d2f4134d5b53957b15e82d4bed24c7eb.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "aparcamientos_silver_to_gold" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aparcamientos_silver_to_gold-b55a4a1d69d1eadf50394eb93b034fd8.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/aparcamientos_silver_to_gold-ce49527d7c8d3dd98bcb65d2ca1b38ad.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "bicimad_bronze_to_silver" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bicimad_bronze_to_silver-cd282dfb63915ca6f80b2ccbd8143809.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bicimad_bronze_to_silver-44a29129cff7226b539da9071dd8f8b7.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "bicimad_gold_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bicimad_backfill_dedup_gold-0eb546a683ffaa467741ed1fa47a2abb.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bicimad_backfill_dedup_gold-3cc7762735e125d2ba40c9a759900087.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "bicimad_silver_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bicimad_backfill_dedup-4a3264e9732202731f31d5974bbe9017.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bicimad_backfill_dedup-ed6e6af42559477339b933051cafe77b.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "bicimad_silver_to_gold" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bicimad_silver_to_gold-dd61f49fdef5e187adf9e3b2cb0bcd68.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bicimad_silver_to_gold-c843909cc91d34dcfdcf321695e074a2.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "bluesky_menciones_bronze_to_silver" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bluesky_menciones_bronze_to_silver-e2d8897c5d4760401b16893568ac32ee.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bluesky_menciones_bronze_to_silver-3911e443483f4b8bccf853ec600d43b6.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "bluesky_menciones_gold_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bluesky_menciones_backfill_dedup_gold-9cfe45ea7aef30a0f20892f1bdaccb0a.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bluesky_menciones_backfill_dedup_gold-eaac70080a579db34ec3c54244f4e426.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "bluesky_menciones_silver_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bluesky_menciones_backfill_dedup-d20c9b44b3da1387c2a6a1d6fd6a5090.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bluesky_menciones_backfill_dedup-2efdec7fc5aa3a4ed77dbee6c1a5e2d4.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "bluesky_menciones_silver_to_gold" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bluesky_menciones_silver_to_gold-8c43f4a9df541e2eea43a8456514dc10.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/bluesky_menciones_silver_to_gold-261976e04868c0265f79a78dacffb6ed.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "calidad_aire_bronze_to_silver" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/calidad_aire_bronze_to_silver-4eb1972a460e5e10ae7df4a3315f52d4.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/calidad_aire_bronze_to_silver-da2056544438c70f538d76fa59f438a4.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "calidad_aire_gold_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/calidad_aire_backfill_dedup_gold-879b3165bb85419fae5a2b8078c723d0.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/calidad_aire_backfill_dedup_gold-b1b779d81465784f5abab97e0cbdab0c.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "calidad_aire_silver_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/calidad_aire_backfill_dedup-6d44949bc6077a4ec6bba66eff619e0e.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/calidad_aire_backfill_dedup-34fd8107fa813f83f6e1b5b6ad747653.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "calidad_aire_silver_to_gold" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/calidad_aire_silver_to_gold-8333a2c7125ffd86e09976ae4db5114d.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/calidad_aire_silver_to_gold-2aa3d2a268bb6c9d89020347d439f194.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "cams_calidad_aire_bronze_to_silver" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/cams_calidad_aire_bronze_to_silver-9211d3802eca398bfda830e10b7b8ef2.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/cams_calidad_aire_bronze_to_silver-c0577843a652fafb4ba8dc477363c545.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "cams_calidad_aire_gold_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/cams_calidad_aire_backfill_dedup_gold-fa45e88fe2d37635cc6240ef327383ff.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/cams_calidad_aire_backfill_dedup_gold-1a9154bc6c0c71a12b1e6c42eea25f30.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "cams_calidad_aire_silver_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/cams_calidad_aire_backfill_dedup-69f636c9df5c3880b98dff5bf4088421.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/cams_calidad_aire_backfill_dedup-f740ec883030bc43077f1cf7c79cffd7.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "cams_calidad_aire_silver_to_gold" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/cams_calidad_aire_silver_to_gold-f83d74685a5a4d930a50993f848d2a01.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/cams_calidad_aire_silver_to_gold-b314b6a6fc7c546a4c090fe2e01f052d.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "cartelera_cines_estrenos_bronze_to_silver" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/cartelera_cines_estrenos_bronze_to_silver-77e98d1cd921c208bf5ffaa29d284e32.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/cartelera_cines_estrenos_bronze_to_silver-2b7b796b05eb81035f181fa7ae643321.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "cartelera_cines_estrenos_silver_to_gold" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/cartelera_cines_estrenos_silver_to_gold-aa6c09b63c18f746da024c09a020b01f.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/cartelera_cines_estrenos_silver_to_gold-8d4592f5bf658249febbacc5cca7df26.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "meteorologia_bronze_to_silver" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/meteorologia_bronze_to_silver-3fcf5c38a2dd24e79206eb53af97348a.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/meteorologia_bronze_to_silver-9f8c92a3c75695ffe310682ba8d437b0.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "meteorologia_gold_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/meteorologia_backfill_dedup_gold-cb6dc670fef14d383aaa366eb184d811.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/meteorologia_backfill_dedup_gold-f919944ff6593ee881f3d4d2a4c57ecf.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "meteorologia_silver_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/meteorologia_backfill_dedup-1fa9eaae33ade611f68b64e9ac2dffc0.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/meteorologia_backfill_dedup-d0fd57ddf99e04744edfcba8d690721c.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "meteorologia_silver_to_gold" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/meteorologia_silver_to_gold-4fb287e094c6f4dd3cb17585cbde692e.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/meteorologia_silver_to_gold-169c14fd98eac491489861d9e5192564.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "ruido_bronze_to_silver" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/ruido_bronze_to_silver-57461bb981d80490227ccb4922409ef9.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/ruido_bronze_to_silver-4578e5651d5577da838113244d5be142.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "ruido_gold_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/ruido_backfill_dedup_gold-db9317465c5f82d4c56c9faae1e83723.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/ruido_backfill_dedup_gold-f687c85eeca0b75468cfe68bb01c48c3.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "ruido_silver_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/ruido_backfill_dedup-2cf7215ae11978fc12206039bba3aece.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/ruido_backfill_dedup-fce8661c6ea351323d8dbec6a79e1377.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "ruido_silver_to_gold" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/ruido_silver_to_gold-77502f7109487420c57b7e41102616e3.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/ruido_silver_to_gold-df06da27741ea0c03b88aab3ac7e0a51.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "trafico_bronze_to_silver" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/trafico_bronze_to_silver-ae01fdf48416d1e59a499e725af5eeb4.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/trafico_bronze_to_silver-168f6d4b74abbb184207e7548dc13bdb.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "trafico_silver_to_gold" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/trafico_silver_to_gold-1884fa42b9e7b491c226ccb77bb38a49.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/trafico_silver_to_gold-60a5f338a6cc68a8c760176719c0db97.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "transporte_publico_emt_bronze_to_silver" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/transporte_publico_emt_bronze_to_silver-5b3c3602b3f60bf9ff3ef5cfe1c8d6a9.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/transporte_publico_emt_bronze_to_silver-227b9894b2fc66730ce2cbb6a7a9f6a3.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "transporte_publico_emt_gold_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/transporte_publico_emt_backfill_dedup_gold-318da358079d2d12e5b8c55e656eb079.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/transporte_publico_emt_backfill_dedup_gold-fe90bd1230ec95accd5efc6836a9c7f5.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "transporte_publico_emt_silver_backfill_dedup" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/transporte_publico_emt_backfill_dedup-961447ee3174a4e4ef33f1b6e006affa.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/transporte_publico_emt_backfill_dedup-23abd9cec70fa40fd164acace5643b81.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_glue_job" "transporte_publico_emt_silver_to_gold" {
      ...
      ~ command {
            name            = "glueetl"
          ~ script_location = "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/transporte_publico_emt_silver_to_gold-7cd0d472adb8dfd5d48cc79dfad6acdb.py" -> "s3://madrono-tfm-dev-build-artifacts-222234418587/glue-scripts/transporte_publico_emt_silver_to_gold-31a48a9152356f0fb479193b278e8d0c.py"
            # (2 unchanged attributes hidden)
        }
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:20:23.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
        tags                           = {}
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:20:41.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:20:11.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:19:59.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:20:05.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:20:29.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:20:53.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:20:47.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:20:35.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:19:42.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:20:59.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:21:05.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:19:36.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:20:17.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:19:48.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
  ...
  ~ resource "aws_lambda_function" "producer" {
      ...
      ~ last_modified                  = "2026-08-28T13:19:54.000+0000" -> (known after apply)
      ~ layers                         = [
          - "arn:aws:lambda:eu-west-1:222234418587:layer:madrono-tfm-dev-ingesta-dependencies:1",
        ]
      ~ source_code_hash               = "4VU1lJW+ldNHGnjruFBEZtFSt2RDVuaRvViONqx75Uw=" -> "qoD8FbheinMCiaupcTJJC61RH8SqsHTNIIFoVPvjhwg="
...
-/+ resource "aws_s3_object" "glue_script_aemet_prevision_avisos_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/aemet_prevision_avisos_bronze_to_silver-d7b98621b0e2b5a6d5d4c70119146f34.py" -> "glue-scripts/aemet_prevision_avisos_bronze_to_silver-5874411ad6f28f158f685ce90841add5.py" # forces replacement
...
-/+ resource "aws_s3_object" "glue_script_aemet_prevision_avisos_silver_to_gold" {
      ...
      ~ key                           = "glue-scripts/aemet_prevision_avisos_silver_to_gold-e203db858089238019b642eb9b9f6a23.py" -> "glue-scripts/aemet_prevision_avisos_silver_to_gold-735e6a4f6938be2c1d4d15ce74735529.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_afluencia_lugares_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/afluencia_lugares_bronze_to_silver-2299183b4c60edf1e2539fc00a59ccc5.py" -> "glue-scripts/afluencia_lugares_bronze_to_silver-e019ce6127891d24af49cb253082177b.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_aforos_peatones_bicicletas_backfill_dedup" {
      ...
      ~ key                           = "glue-scripts/aforos_peatones_bicicletas_backfill_dedup-bfb76e782afee2c5956f548a34da0b18.py" -> "glue-scripts/aforos_peatones_bicicletas_backfill_dedup-8dc97077e9a7b0d793edb42e68c8a090.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_aforos_peatones_bicicletas_backfill_dedup_gold" {
      ...
      ~ key                           = "glue-scripts/aforos_peatones_bicicletas_backfill_dedup_gold-7c312ea88152d382dd1e02b82a549a7c.py" -> "glue-scripts/aforos_peatones_bicicletas_backfill_dedup_gold-dedd44063c766e8475986650df404498.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_aforos_peatones_bicicletas_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/aforos_peatones_bicicletas_bronze_to_silver-d8325aae3ee77630cfc0f6612c30323e.py" -> "glue-scripts/aforos_peatones_bicicletas_bronze_to_silver-d037fa3c1d50aa6cdf1a4355e4af910b.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_aforos_peatones_bicicletas_silver_to_gold" {
      ...
      ~ key                           = "glue-scripts/aforos_peatones_bicicletas_silver_to_gold-a7df88f8aa89cea895c3e594ff738600.py" -> "glue-scripts/aforos_peatones_bicicletas_silver_to_gold-98ae6a2fa1ca9fc05b6451aaffbd690b.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_agenda_eventos_backfill_dedup" {
      ...
      ~ key                           = "glue-scripts/agenda_eventos_backfill_dedup-ebb7fb05697677064a5b18ee492aca9e.py" -> "glue-scripts/agenda_eventos_backfill_dedup-406949108f74a212bfa3dd0a5f67acca.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_agenda_eventos_backfill_dedup_gold" {
      ...
      ~ key                           = "glue-scripts/agenda_eventos_backfill_dedup_gold-c417e4a5711e3fcb2d416cc0f05f3290.py" -> "glue-scripts/agenda_eventos_backfill_dedup_gold-b4c8693ee116e2f50aee1b96fd7018c6.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_agenda_eventos_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/agenda_eventos_bronze_to_silver-75c29ecd15eb33bf665840234bcf5cc8.py" -> "glue-scripts/agenda_eventos_bronze_to_silver-e24cfdad3be04dfe065231b9643719c0.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_agenda_eventos_silver_to_gold" {
      ...
      ~ key                           = "glue-scripts/agenda_eventos_silver_to_gold-4d854fda4df74abc055b9eba9af92b0f.py" -> "glue-scripts/agenda_eventos_silver_to_gold-7ed6c1455ead3aef19f9e40b96c23a51.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_aparcamientos_backfill_dedup" {
      ...
      ~ key                           = "glue-scripts/aparcamientos_backfill_dedup-0040b8ac53f09f609005c2ad2aac464f.py" -> "glue-scripts/aparcamientos_backfill_dedup-d2f4134d5b53957b15e82d4bed24c7eb.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_aparcamientos_backfill_dedup_gold" {
      ...
      ~ key                           = "glue-scripts/aparcamientos_backfill_dedup_gold-7f4c18ec21a262d4a6e788348e492c3f.py" -> "glue-scripts/aparcamientos_backfill_dedup_gold-b03433c6e4e72c7e33e557790f1809b2.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_aparcamientos_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/aparcamientos_bronze_to_silver-4c9fe8e66729a98c520c97a0aa10f630.py" -> "glue-scripts/aparcamientos_bronze_to_silver-90e5ef17131a899ea2f70fcff0bb1962.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_aparcamientos_silver_to_gold" {
      ...
      ~ key                           = "glue-scripts/aparcamientos_silver_to_gold-b55a4a1d69d1eadf50394eb93b034fd8.py" -> "glue-scripts/aparcamientos_silver_to_gold-ce49527d7c8d3dd98bcb65d2ca1b38ad.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_bicimad_backfill_dedup" {
      ...
      ~ key                           = "glue-scripts/bicimad_backfill_dedup-4a3264e9732202731f31d5974bbe9017.py" -> "glue-scripts/bicimad_backfill_dedup-ed6e6af42559477339b933051cafe77b.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_bicimad_backfill_dedup_gold" {
      ...
      ~ key                           = "glue-scripts/bicimad_backfill_dedup_gold-0eb546a683ffaa467741ed1fa47a2abb.py" -> "glue-scripts/bicimad_backfill_dedup_gold-3cc7762735e125d2ba40c9a759900087.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_bicimad_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/bicimad_bronze_to_silver-cd282dfb63915ca6f80b2ccbd8143809.py" -> "glue-scripts/bicimad_bronze_to_silver-44a29129cff7226b539da9071dd8f8b7.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_bicimad_silver_to_gold" {
      ...
      ~ key                           = "glue-scripts/bicimad_silver_to_gold-dd61f49fdef5e187adf9e3b2cb0bcd68.py" -> "glue-scripts/bicimad_silver_to_gold-c843909cc91d34dcfdcf321695e074a2.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_bluesky_menciones_backfill_dedup" {
      ...
      ~ key                           = "glue-scripts/bluesky_menciones_backfill_dedup-d20c9b44b3da1387c2a6a1d6fd6a5090.py" -> "glue-scripts/bluesky_menciones_backfill_dedup-2efdec7fc5aa3a4ed77dbee6c1a5e2d4.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_bluesky_menciones_backfill_dedup_gold" {
      ...
      ~ key                           = "glue-scripts/bluesky_menciones_backfill_dedup_gold-9cfe45ea7aef30a0f20892f1bdaccb0a.py" -> "glue-scripts/bluesky_menciones_backfill_dedup_gold-eaac70080a579db34ec3c54244f4e426.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_bluesky_menciones_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/bluesky_menciones_bronze_to_silver-e2d8897c5d4760401b16893568ac32ee.py" -> "glue-scripts/bluesky_menciones_bronze_to_silver-3911e443483f4b8bccf853ec600d43b6.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_bluesky_menciones_silver_to_gold" {
      ...
      ~ key                           = "glue-scripts/bluesky_menciones_silver_to_gold-8c43f4a9df541e2eea43a8456514dc10.py" -> "glue-scripts/bluesky_menciones_silver_to_gold-261976e04868c0265f79a78dacffb6ed.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/trafico_bronze_to_silver-ae01fdf48416d1e59a499e725af5eeb4.py" -> "glue-scripts/trafico_bronze_to_silver-168f6d4b74abbb184207e7548dc13bdb.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_calidad_aire_backfill_dedup" {
      ...
      ~ key                           = "glue-scripts/calidad_aire_backfill_dedup-6d44949bc6077a4ec6bba66eff619e0e.py" -> "glue-scripts/calidad_aire_backfill_dedup-34fd8107fa813f83f6e1b5b6ad747653.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_calidad_aire_backfill_dedup_gold" {
      ...
      ~ key                           = "glue-scripts/calidad_aire_backfill_dedup_gold-879b3165bb85419fae5a2b8078c723d0.py" -> "glue-scripts/calidad_aire_backfill_dedup_gold-b1b779d81465784f5abab97e0cbdab0c.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_calidad_aire_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/calidad_aire_bronze_to_silver-4eb1972a460e5e10ae7df4a3315f52d4.py" -> "glue-scripts/calidad_aire_bronze_to_silver-da2056544438c70f538d76fa59f438a4.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_calidad_aire_silver_to_gold" {
      ...
      ~ key                           = "glue-scripts/calidad_aire_silver_to_gold-8333a2c7125ffd86e09976ae4db5114d.py" -> "glue-scripts/calidad_aire_silver_to_gold-2aa3d2a268bb6c9d89020347d439f194.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_cams_calidad_aire_backfill_dedup" {
      ...
      ~ key                           = "glue-scripts/cams_calidad_aire_backfill_dedup-69f636c9df5c3880b98dff5bf4088421.py" -> "glue-scripts/cams_calidad_aire_backfill_dedup-f740ec883030bc43077f1cf7c79cffd7.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_cams_calidad_aire_backfill_dedup_gold" {
      ...
      ~ key                           = "glue-scripts/cams_calidad_aire_backfill_dedup_gold-fa45e88fe2d37635cc6240ef327383ff.py" -> "glue-scripts/cams_calidad_aire_backfill_dedup_gold-1a9154bc6c0c71a12b1e6c42eea25f30.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_cams_calidad_aire_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/cams_calidad_aire_bronze_to_silver-9211d3802eca398bfda830e10b7b8ef2.py" -> "glue-scripts/cams_calidad_aire_bronze_to_silver-c0577843a652fafb4ba8dc477363c545.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_cams_calidad_aire_silver_to_gold" {
      ...
      ~ key                           = "glue-scripts/cams_calidad_aire_silver_to_gold-f83d74685a5a4d930a50993f848d2a01.py" -> "glue-scripts/cams_calidad_aire_silver_to_gold-b314b6a6fc7c546a4c090fe2e01f052d.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_cartelera_cines_estrenos_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/cartelera_cines_estrenos_bronze_to_silver-77e98d1cd921c208bf5ffaa29d284e32.py" -> "glue-scripts/cartelera_cines_estrenos_bronze_to_silver-2b7b796b05eb81035f181fa7ae643321.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_cartelera_cines_estrenos_silver_to_gold" {
      ...
      ~ key                           = "glue-scripts/cartelera_cines_estrenos_silver_to_gold-aa6c09b63c18f746da024c09a020b01f.py" -> "glue-scripts/cartelera_cines_estrenos_silver_to_gold-8d4592f5bf658249febbacc5cca7df26.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_meteorologia_backfill_dedup" {
      ...
      ~ key                           = "glue-scripts/meteorologia_backfill_dedup-1fa9eaae33ade611f68b64e9ac2dffc0.py" -> "glue-scripts/meteorologia_backfill_dedup-d0fd57ddf99e04744edfcba8d690721c.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_meteorologia_backfill_dedup_gold" {
      ...
      ~ key                           = "glue-scripts/meteorologia_backfill_dedup_gold-cb6dc670fef14d383aaa366eb184d811.py" -> "glue-scripts/meteorologia_backfill_dedup_gold-f919944ff6593ee881f3d4d2a4c57ecf.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_meteorologia_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/meteorologia_bronze_to_silver-3fcf5c38a2dd24e79206eb53af97348a.py" -> "glue-scripts/meteorologia_bronze_to_silver-9f8c92a3c75695ffe310682ba8d437b0.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_meteorologia_silver_to_gold" {
      ...
      ~ key                           = "glue-scripts/meteorologia_silver_to_gold-4fb287e094c6f4dd3cb17585cbde692e.py" -> "glue-scripts/meteorologia_silver_to_gold-169c14fd98eac491489861d9e5192564.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_ruido_backfill_dedup" {
      ...
      ~ key                           = "glue-scripts/ruido_backfill_dedup-2cf7215ae11978fc12206039bba3aece.py" -> "glue-scripts/ruido_backfill_dedup-fce8661c6ea351323d8dbec6a79e1377.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_ruido_backfill_dedup_gold" {
      ...
      ~ key                           = "glue-scripts/ruido_backfill_dedup_gold-db9317465c5f82d4c56c9faae1e83723.py" -> "glue-scripts/ruido_backfill_dedup_gold-f687c85eeca0b75468cfe68bb01c48c3.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_ruido_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/ruido_bronze_to_silver-57461bb981d80490227ccb4922409ef9.py" -> "glue-scripts/ruido_bronze_to_silver-4578e5651d5577da838113244d5be142.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_ruido_silver_to_gold" {
      ...
      ~ key                           = "glue-scripts/ruido_silver_to_gold-77502f7109487420c57b7e41102616e3.py" -> "glue-scripts/ruido_silver_to_gold-df06da27741ea0c03b88aab3ac7e0a51.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_silver_to_gold" {
      ...
      ~ key                           = "glue-scripts/trafico_silver_to_gold-1884fa42b9e7b491c226ccb77bb38a49.py" -> "glue-scripts/trafico_silver_to_gold-60a5f338a6cc68a8c760176719c0db97.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_transporte_publico_emt_backfill_dedup" {
      ...
      ~ key                           = "glue-scripts/transporte_publico_emt_backfill_dedup-961447ee3174a4e4ef33f1b6e006affa.py" -> "glue-scripts/transporte_publico_emt_backfill_dedup-23abd9cec70fa40fd164acace5643b81.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_transporte_publico_emt_backfill_dedup_gold" {
      ...
      ~ key                           = "glue-scripts/transporte_publico_emt_backfill_dedup_gold-318da358079d2d12e5b8c55e656eb079.py" -> "glue-scripts/transporte_publico_emt_backfill_dedup_gold-fe90bd1230ec95accd5efc6836a9c7f5.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_transporte_publico_emt_bronze_to_silver" {
      ...
      ~ key                           = "glue-scripts/transporte_publico_emt_bronze_to_silver-5b3c3602b3f60bf9ff3ef5cfe1c8d6a9.py" -> "glue-scripts/transporte_publico_emt_bronze_to_silver-227b9894b2fc66730ce2cbb6a7a9f6a3.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "glue_script_transporte_publico_emt_silver_to_gold" {
      ...
      ~ key                           = "glue-scripts/transporte_publico_emt_silver_to_gold-7cd0d472adb8dfd5d48cc79dfad6acdb.py" -> "glue-scripts/transporte_publico_emt_silver_to_gold-31a48a9152356f0fb479193b278e8d0c.py" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
-/+ resource "aws_s3_object" "layer_build_source" {
      + acl                           = (known after apply)
      ~ arn                           = "arn:aws:s3:::madrono-tfm-dev-build-artifacts-222234418587/source/ingesta-requirements-c25e831d1b28851d8cdce0447b7a3aae.zip" -> (known after apply)
      ~ bucket_key_enabled            = false -> (known after apply)
      + checksum_crc32                = (known after apply)
      + checksum_crc32c               = (known after apply)
      + checksum_crc64nvme            = (known after apply)
      ...
      + checksum_sha256               = (known after apply)
      ~ content_type                  = "application/octet-stream" -> (known after apply)
      ~ etag                          = "edb7bd12f7cac35fa68c9ebfb066f8a3" -> "936f95f99fde7bf3f5b493d925d97f1f"
      ~ id                            = "source/ingesta-requirements-c25e831d1b28851d8cdce0447b7a3aae.zip" -> (known after apply)
      ~ key                           = "source/ingesta-requirements-c25e831d1b28851d8cdce0447b7a3aae.zip" -> "source/ingesta-requirements-65d5d59fef3021bc0831a29454145c7b.zip" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ storage_class                 = "STANDARD" -> (known after apply)
      - tags                          = {} -> null
...
-/+ resource "aws_s3_object" "procesamiento_source" {
      + acl                           = (known after apply)
      ~ arn                           = "arn:aws:s3:::madrono-tfm-dev-build-artifacts-222234418587/glue-libs/procesamiento-a7ba99ac9375c8f81e041af4185355c9.zip" -> (known after apply)
      ~ bucket_key_enabled            = false -> (known after apply)
      + checksum_crc32                = (known after apply)
      + checksum_crc32c               = (known after apply)
      + checksum_crc64nvme            = (known after apply)
      ...
      ~ id                            = "glue-libs/procesamiento-a7ba99ac9375c8f81e041af4185355c9.zip" -> (known after apply)
      ~ key                           = "glue-libs/procesamiento-a7ba99ac9375c8f81e041af4185355c9.zip" -> "glue-libs/procesamiento-38ac40cf3a7a2950bd8beefe827db5b5.zip" # forces replacement
      + kms_key_id                    = (known after apply)
      - metadata                      = {} -> null
      ~ server_side_encryption        = "AES256" -> (known after apply)
      ~ storage_class                 = "STANDARD" -> (known after apply)
...
Plan: 49 to add, 66 to change, 49 to destroy.
