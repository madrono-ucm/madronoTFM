...
<style>
  ...
  .panel { position:absolute; z-index:5; background:rgba(18,22,28,.95); color:#e8edf2;
    backdrop-filter:blur(4px); border:1px solid #2c3644; border-radius:10px;
    padding:10px 12px; font-size:13px; box-shadow:0 6px 24px rgba(0,0,0,.35); }
  #titulo { left:50%; top:10px; transform:translateX(-50%); padding:6px 16px; font-weight:650;
    letter-spacing:.2px; text-align:center; z-index:6; }
  #titulo small { display:block; font-weight:400; color:#9fb0c0; font-size:11px; margin-top:2px; }
  /* Barra lateral de secciones: un botón por panel; el panel se muestra u
     oculta al pulsarlo (una sección visible a la vez). */
  #rail { left:12px; top:56px; z-index:7; display:flex; flex-direction:column; gap:3px;
    padding:6px; }
  #rail button { display:block; width:120px; text-align:left; font-size:12px; font-weight:600;
    padding:6px 10px; background:transparent; border:1px solid transparent; color:#c8d4df; }
  #rail button:hover { background:rgba(255,255,255,.05); }
  #rail button.on { background:#3d6ce0; border-color:#3d6ce0; color:#fff; }
  #controls { left:150px; top:56px; width:280px; max-height:calc(100% - 210px); overflow:auto; }
  #controls.hidden { display:none; }
  .sec { display:none; }
  .sec.on { display:block; }
  .sec > h3 { margin:0 0 8px; font-size:13px; font-weight:650; letter-spacing:.2px; color:#e8edf2; }
  #context  { right:12px; top:56px; width:250px; max-height:calc(100% - 200px); overflow:auto; }
  @media (max-width:640px){
    #rail { flex-direction:row; flex-wrap:wrap; width:min(94vw,340px); }
    #rail button { width:auto; }
    #controls { left:12px; top:150px; width:min(94vw,340px); }
  }
...
function layers(){
  ...
  if(!state.clean){
    // Con un nodo seleccionado, las aristas que lo tocan van a plena
    // intensidad y el resto se atenúan — así se lee qué conecta con qué.
    const arcOn = d => selNode==null || d.s===selNode || d.t===selNode;
    const arcTint = (d,v) => { const c = arcCol(v).slice(); if(!arcOn(d)) c[3] = Math.round(c[3]*0.16); return c; };
    L.push(new ArcLayer({id:"imp", data:META.arcs,
      getSourcePosition:d=>META.coords[d.s], getTargetPosition:d=>META.coords[d.t],
      getSourceColor:d=>arcTint(d, trafNow[d.s]), getTargetColor:d=>arcTint(d, trafNow[d.t]),
      getWidth:d=>(0.8+3*d.w)*(arcOn(d)?1:0.45), getHeight:0.16,
      updateTriggers:{
        getSourceColor:[state.day,state.hour,state.hz,selNode],
        getTargetColor:[state.day,state.hour,state.hz,selNode],
        getWidth:[selNode]}}));
  }
...
// Cuántas de las 15 aristas más influyentes del STGNN tocan cada nodo.
// `importancia_aristas` es un top-15 GLOBAL, no una adyacencia por nodo:
// #5768 sale en 8 y #4848 en 6 (son los concentradores de flujo del
// modelo), el resto en 1. Memoizado.
let _impDeg = null;
function impDeg(){
  if(_impDeg) return _impDeg;
  _impDeg = {};
  (META.arcs||[]).forEach(a=>{ _impDeg[a.s]=(_impDeg[a.s]||0)+1; _impDeg[a.t]=(_impDeg[a.t]||0)+1; });
  return _impDeg;
}
const HUB_MIN = 3;
...
function edgePane(){
  ...
  const box = document.getElementById("ea-imp");
  box.textContent = "";
  const mine = META.arcs
    .filter(a=>a.s===selNode||a.t===selNode)
    .map(a=>({ otro:(a.s===selNode?a.t:a.s), dir:(a.s===selNode?"influye":"influido por"), w:a.w }))
    .sort((x,y)=>y.w-x.w);
  if(!mine.length){
    box.className = "muted";
  ...
  box.className = "";
  const n = impDeg()[selNode] || mine.length;
  ...
  box.appendChild(cab);
  mine.forEach(m=>{
    const nid = META.node_id ? META.node_id[m.otro] : m.otro;
    const pct = Math.max(4, Math.round(m.w*100));
    const r = document.createElement("div");
    r.style.cssText = "display:flex;align-items:center;gap:6px;margin:2px 0;font-size:11px";
    r.innerHTML =
      "<span style='color:#c8d4df;white-space:nowrap'>"+(m.dir==="influye"?"→":"←")+" #"+nid+"</span>"
     +"<span style='color:#9fb0c0;white-space:nowrap'>"+m.dir+"</span>"
     +"<span style='flex:1;height:6px;background:#2c3644;border-radius:3px'>"
     +"<span style='display:block;height:100%;border-radius:3px;width:"+pct+"%;background:#7fd1ff'></span></span>"
     +"<span style='color:#9fb0c0;width:26px;text-align:right'>"+m.w.toFixed(2)+"</span>";
    box.appendChild(r);
  });
}
...
function fitBounds(){
  // META.bbox = [[oeste, sur], [este, norte]] — el formato que espera maplibre.
  map.fitBounds(META.bbox, {padding:60, duration:0});
}

// ---- Recorrido guiado (storytelling): cada capítulo pone el mapa en el
//      estado que ilustra un hallazgo del TFM ------------------------------
function rutaIdx(o,d,perfil){
...
function setEstado(p){
  ...
  document.querySelectorAll(".es").forEach(x=>x.classList.toggle("on", x.dataset.e===state.escala));
  document.querySelectorAll(".pf").forEach(x=>x.classList.toggle("on", x.dataset.p===state.perfil));
  const gh = document.getElementById("ghost"); if(gh) gh.classList.toggle("on", state.ghost);
  const hr = document.getElementById("hour"); if(hr) hr.value = state.hour;
  const odSel = document.getElementById("r-od"), pSel = document.getElementById("r-perfil");
  if(RUTAS && state.route>=0 && RUTAS.rutas[state.route]){
    const R = RUTAS.rutas[state.route];
    const ods = [...new Set(RUTAS.rutas.map(r=>r.origen+" → "+r.destino))];
    odSel.value = ods.indexOf(R.origen+" → "+R.destino); pSel.value = R.perfil;
  } else if(odSel){ odSel.value = -1; }
  render();
}
const CAPITULOS = [
...
let histIdx = -1;
function irCapitulo(i){
  histIdx = ((i % CAPITULOS.length) + CAPITULOS.length) % CAPITULOS.length;
  const c = CAPITULOS[histIdx];
  document.querySelectorAll("#hist-caps button").forEach(b=>b.classList.toggle("on", +b.dataset.i===histIdx));
  document.getElementById("hist-texto").innerHTML = "<b>"+c.t+"</b><br>"+c.d;
  c.a();
}
function mkHistoria(){
  const wrap = document.getElementById("hist-caps");
  CAPITULOS.forEach((c,i)=>{ const b=document.createElement("button");
    b.textContent = String(i+1); b.title = c.t; b.dataset.i = i;
    b.onclick = ()=>irCapitulo(i); wrap.appendChild(b); });
  document.getElementById("hist-prev").onclick = ()=>irCapitulo(histIdx-1);
  document.getElementById("hist-next").onclick = ()=>irCapitulo(histIdx+1);
  document.getElementById("hist-auto").onclick = ()=>{
    state.playing = !state.playing;
    document.getElementById("hist-auto").classList.toggle("on", state.playing);
    const pb = document.getElementById("play"); if(pb) pb.classList.toggle("on", state.playing);
    if(state.playing) tick();
  };
}

// ---- Chat con el asistente (LLM + herramientas MCP sobre grafo/Gold) -------
// La página es estática (gh-pages); apunta al asistente desplegado. Se puede
...
// modelo respondió sin consultar datos.
function chatTraza(pasos){
  if(!Array.isArray(pasos)) return;
  const h = document.getElementById("chat-hilo");
  const d = document.createElement("div");
  d.style.cssText = "margin:1px 0 5px;font-size:10.5px;color:#8a97a6";
  d.textContent = pasos.length
    ? "🔧 " + pasos.map(p => p.tool + (p.ok ? "" : " (falló)")).join(" · ")
    : "· respondido sin consultar datos";
  h.appendChild(d); h.scrollTop = 1e9;
}
async function chatEnviar(){
  const inp = document.getElementById("chat-in"), est = document.getElementById("chat-estado");
  const q = inp.value.trim(); if(!q) return;
  inp.value = ""; chatMsg("user", q); est.textContent = "pensando…";
  document.getElementById("chat-send").disabled = true;
  try{
    const r = await fetch(API_BASE + "/chat", {method:"POST",
      headers:{"content-type":"application/json", ...AUTH_DEMO},
      body: JSON.stringify({mensaje:q, historial:chatHist})});
    if(!r.ok) throw new Error("HTTP "+r.status);
    const j = await r.json();
    chatHist = j.historial || [];
    chatMsg("bot", j.respuesta || "(sin respuesta)");
    chatTraza(j.pasos);
  }catch(err){
    chatMsg("bot", "no se pudo consultar el asistente ("+err+")."
      + (API_BASE ? "" : " Sirve esta página desde el asistente o añade ?api=<url>."));
  }
  est.textContent = ""; document.getElementById("chat-send").disabled = false;
}
function chatSugerencias(lista){
  const sug = document.getElementById("chat-sug");
  sug.innerHTML = "";
  lista.forEach(s=>{ const q = s.ejemplo || s, t = s.titulo || s;
    const b=document.createElement("button");
    b.textContent = q.length>26 ? q.slice(0,24)+"…" : q; b.title = t; b.style.fontSize = "10.5px";
    b.onclick = ()=>{ document.getElementById("chat-in").value = q; chatEnviar(); };
    sug.appendChild(b); });
}
async function mkChat(){
  chatSugerencias(CHAT_SUG);
...
// Un botón por sección; al pulsarlo se muestra su panel y se ocultan los
// demás (una sección visible a la vez). Pulsar el botón activo lo cierra.
function mkRail(){
  const rail = document.getElementById("rail");
  const cont = document.getElementById("controls");
  const secs = cont.querySelectorAll(".sec");
  rail.querySelectorAll("button").forEach(b=>{
    b.setAttribute("aria-expanded", b.classList.contains("on") ? "true" : "false");
    b.onclick = ()=>{
      const abrir = !b.classList.contains("on");
      rail.querySelectorAll("button").forEach(x=>{ x.classList.remove("on"); x.setAttribute("aria-expanded","false"); });
      ...
      if(abrir){
        ...
        const s = cont.querySelector('.sec[data-sec="'+b.dataset.sec+'"]');
        if(s) s.classList.add("on");
        if(b.dataset.sec==="chat"){ const inp=document.getElementById("chat-in"); if(inp) inp.focus(); }
      }
    };
  });
}
...
function mkControls(){
  ...
  document.getElementById("clean").onclick=e=>{
    ...
    document.getElementById("rail").style.opacity = state.clean ? ".22" : "1";
...
Promise.all([
  ...
  mkControls(); mkHistoria(); mkChat(); mkRail(); syncTabs();
  map.on("load", ()=>{ if(META.bbox) fitBounds(); render(); });
}).catch(e=>{ document.getElementById("map").innerHTML =
  "<p style='color:#ccc;font:14px system-ui;padding:24px'>No se pudo montar el mapa. "
