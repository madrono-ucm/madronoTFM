...
function routeInfo(){
  ...
  const red = r.reduccion_exposicion_pct;
  el.innerHTML = `E3 · <b>${R.origen}→${R.destino}</b> [${R.perfil}] · verde = saludable, gris = rápida<br>`
    + `+${r.delta_dist_pct}% distancia · exposición −: tráf ${red.traf}% / NO₂ ${red.no2}% / O₃ ${red.o3}% / ruido ${red.noise}%`
    + ` · mejor hora salida: ${R.mejor_hora}:00`;
}
