"""FIL_34 (M3) + FIL_43 + FIL_47 — el mapa animado del grafo de Madrid.
...
- `viz/mapa/index.html`  — deck.gl (CDN) sobre los polígonos de distrito
  ...
  de color / Vista / Ruta); cámara 2D/3D + "encajar a Madrid"; etiquetas de
  distrito, hitos, ejes estructurantes (contexto) y parques como capas
  conmutables; tooltip por nodo; barra de título; leyenda pegada a su
  selector. E1 arcos de importancia, E2 ghost, E3 ruta (2 desplegables),
  E4 panel de nodo, E6 pulso de distrito. Carga `data.json` / `meta.json` /
  `weather.json` / `rutas.json` por `fetch`.
...
<style>
  ...
  #controls { left:12px; top:56px; width:272px; max-height:calc(100% - 68px); overflow:auto; }
  #context  { right:12px; top:56px; width:250px; max-height:calc(100% - 68px); overflow:auto; }
  #ticker   { left:12px; bottom:12px; font-variant-numeric:tabular-nums; }
...
<script>
const {DeckGL, ScatterplotLayer, LineLayer, ArcLayer, GeoJsonLayer, PathLayer, TextLayer, WebMercatorViewport} = deck;
