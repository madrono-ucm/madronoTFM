...
| **M4c** | `FIL_47` | ✅ hecho | 2026-08-31 | **Legibilidad**: layout en 4 grupos colapsables + barra de título; cámara **2D/3D** + **"encajar a Madrid"** (`fitBounds` sobre `bbox`); `TextLayer` de nombres de distrito (21 centroides), marcadores + etiqueta de los **14 hitos** (Plaza Elíptica), acentos/ñ vía `characterSet:"auto"`+sdf; **tooltip** por nodo; ejes estructurantes (`viz/assets/ejes_madrid.geojson`, aprox, contexto) y **parques** (`viz/assets/parques_madrid.geojson`, 16 grandes de Bronce) como capas conmutables; ruta E3 a **2 desplegables** + etiquetas de origen/destino; `<html lang="es">` + foco visible + `aria-label`. `tests/` 35. Republicado en `gh-pages`. **No** añade tiles ni arregla G9. |
