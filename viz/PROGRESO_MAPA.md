...
| URL publicada | **https://madrono-ucm.github.io/madronoTFM/** (rama `gh-pages`, `FIL_42`) · explorador del grafo en vivo: **https://35-43-165-117.nip.io/grafo/explorador** (`demo`/`demo`, `FIL_67`/`FIL_68`) · local: `python -m http.server -d viz/mapa` |
|---|---|
...
| **M4b** | `FIL_45` | ✅ hecho | 2026-08-31 | **Capa social y de accesibilidad**: grupo **♿ Salud · perfil de sensibilidad** con **9 perfiles** (general/ciclista/sensible_aire/sensible_ruido + asma_epoc/mayor/infancia/movilidad_reducida/trabajo_exterior, mismos pesos que `ruta_saludable`, `FIL_37`), métrica **`salud (perfil)`** ponderada en el navegador. Escala **lineal / bandas OMS·UE** (color discreto por umbral, leyenda que **nombra** la banda; paleta apta deuteranopía). Métricas **`dosis NO₂` / `dosis O₃`** (media 8 h como % de guía OMS). **"Mejor hora hoy"** por perfil (barrido 24 h). **Confianza IDW** como capa opcional (nodos lejos de las 11 estaciones, gap G4). Guardarraíles siempre visibles: agregados por zona, sin datos personales, describe el aire y la hora no señala barrios, apoyo a la decisión no consejo médico. UI español + `:focus-visible` + `aria-label`. `tests/` 40. Republicado en `gh-pages`. **No** hace análisis distribucional formal (encuadre en `FIL_36`). |
| **M4c** | `FIL_47` | ✅ hecho | 2026-08-31 | **Legibilidad**: layout en 4 grupos colapsables + barra de título; cámara **2D/3D** + **"encajar a Madrid"** (`fitBounds` sobre `bbox`); `TextLayer` de nombres de distrito (21 centroides), marcadores + etiqueta de los **14 hitos** (Plaza Elíptica), acentos/ñ vía `characterSet:"auto"`+sdf; **tooltip** por nodo; ejes estructurantes (`viz/assets/ejes_madrid.geojson`, aprox, contexto) y **parques** (`viz/assets/parques_madrid.geojson`, 16 grandes de Bronce) como capas conmutables; ruta E3 a **2 desplegables** + etiquetas de origen/destino; `<html lang="es">` + foco visible + `aria-label`. **No** añade tiles ni arregla G9. |
| **M4c** | `FIL_48` | ✅ hecho | 2026-08-31 | **Pulido visual + clic en nodo** (bug de `FIL_47`): `onClick` del nodo restaurado + anillo de selección `#sel`; textura del grafo **off por defecto**; radio de nodo dependiente del zoom; nodos sin dato casi invisibles (α28); distritos/arcos más tenues y planos; etiquetas en MAYÚSCULAS más pequeñas; fondo con degradado radial; botón **"vista limpia"**. `viz/mapa_frames.png` rehecha en oscuro, 5 fotogramas legibles. `tests/` 36. Republicado en `gh-pages`. |
| **M4c** | `FIL_49` | ✅ hecho | 2026-08-31 | **Barras 3D + panel de resumen**: `ColumnLayer` con altura = "gravedad" (sube donde las condiciones son peores) + selector **auto/puntos/barras** (auto = barras en 3D) → el pitch 3D por fin dice algo. **Panel inferior** (`#resumen`, SVG en la propia página): media ciudad 24 h de la métrica actual + barras por distrito ahora + meteo/skill. La capa "textura" pasa a ser el grafo real completo (8.758 aristas). Descartado: basemap híbrido (→ `FIL_50`, opt-in) y PyViz (necesitaría servidor). `tests/` 38. Republicado en `gh-pages`. |
| **M4c** | `FIL_50` | ✅ hecho | 2026-08-31 | **Basemap vectorial opcional** (opt-in): `maplibre-gl@4.7.1` por CDN; selector `#basemap` en 🧭 Vista con **ninguno** (por defecto, estilo vacío = aspecto actual, cero tiles) · Carto **Positron / Dark Matter / Voyager** (sin token). `dgl.setProps({mapStyle})` al cambiar. **Degradación elegante**: sin `maplibregl` el selector queda `disabled` y el mapa sigue siendo el `DeckGL` plano; la tira PNG y el modo offline no dependen de esto. **No** añade satélite/híbrido real (necesitaría tiles con token). `tests/` 41. Republicado en `gh-pages`. |
| — | `FIL_46` | ✅ parcial | 2026-08-31 | **`mejor_hora_zona`, 14.ª tool MCP**: responde "tengo asma, ¿cuándo paseo hoy por Vallecas?" — resuelve la zona por texto libre a distrito (nombre / `id` / alias coloquiales; ambigüedad → lista) y hace el barrido "mejor hora hoy" por distrito + perfil (exposición media de los nodos, 9 perfiles de `FIL_45`) → `mejor_hora`, `peor_hora`, franja limpia, `serie_horaria` 24 h. `asistente/mejor_hora_zona.py` (Python puro sobre `grafo_ruta.json`) + `MejorHoraZona` + router `GET /mejor-hora-zona` + 17 tests; `asistente/` 162 verde. **Sigue como encuadre**: el canal de notificación + la política de umbral por distrito de las *alertas anticipadas*. Se cita en `FIL_36`. |
...
| perf | `FIL_76` | ✅ hecho | 2026-09-10 | **Optimización de carga del mapa publicado**: deck.gl+maplibre salen del `<head>` (van al final del `<body>`) → **FCP 3,0 s → 0,14 s**; `<head>` con `preconnect`/`preload` de `meta.json`+`data.json`; `data.json` −11 % (clave `trafico` era idéntica a `traf_h1`, se deja de emitir); CDN `unpkg` → `cdn.jsdelivr.net` (brotli, mejor SLA); basemap por defecto **Voyager → Positron** (claro, para viz de datos). `tests/` 16 + jsdom 3 verdes. Republicado en `gh-pages` (incluye el recorrido guiado de `FIL_69`). |
